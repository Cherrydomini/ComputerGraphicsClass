...
screen = pygame.display.set_mode((width, height), 0, 32)
pygame.display.set_caption("Trailing Dots and Polyline MouseButton")
...
while not done:
    ...
    if old_pt[0] != 0 and old_pt[1] != 0:
        """p2 = np.array([old_pt[0], cur_pt[1]])"""
        pygame.draw.line(screen, GREEN, old_pt, cur_pt, 5)
        pygame.draw.circle(screen, RED, (cur_pt[0] - 15, cur_pt[1] - 10), 1, 1)
        pygame.draw.circle(screen, RED, (cur_pt[0] - 5, cur_pt[1] - 0), 1, 1)
        pygame.draw.circle(screen, RED, (cur_pt[0] + 5, cur_pt[1] + 15), 1, 1)


    if pressed == 1:
        if button1 == 1:
            pygame.draw.rect(screen, BLUE, (cur_pt[0], cur_pt[1], 2 * margin, 2 * margin), 5)
            p2 = pygame.mouse.get_pos()
            p1 = cur_pt[0], cur_pt[1]
            pygame.draw.rect(screen, BLUE, (cur_pt[0], cur_pt[1], 2 * margin, 2 * margin), 5)
        elif button3 == 1:
